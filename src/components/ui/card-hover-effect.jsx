...
export const HoverEffect = ({ items, className }) => {
  ...
  return (
    ...
    >
      {items.map((item, idx) => (
        <Link
          to={item?.link} // Corrected prop name
          key={item?.link}
...
const Card = ({ className, children }) => {
  return (
    <div
      className={cn(
        "rounded-2xl h-80 w-full overflow-hidden bg-[#181818] border border-transparent dark:border-white/[0.2] group-hover:border-slate-700 relative z-20",
        className
      )}
    >
